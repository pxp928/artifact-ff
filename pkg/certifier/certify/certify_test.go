...
import (
	...
	proc_test "github.com/guacsec/guac/internal/testing/processor"
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if err == nil {
				for i := range collectedDocs {
					result, err := proc_test.DocEqualWithTimestamp(collectedDocs[i], tt.want[i])
					if err != nil {
						t.Error(err)
					}
					if !result {
...
func Test_Publish(t *testing.T) {
	expectedDocTree := proc_test.DocNode(&testdata.Ite6SLSADoc)
	...
	for subscribersDone < numSubscribers {
		select {
		case d := <-docChan:
			if !proc_test.DocTreeEqual(d, expectedDocTree) {
				t.Errorf("doc tree did not match up, got\n%s, \nexpected\n%s", proc_test.StringTree(d), proc_test.StringTree(expectedDocTree))
			}
		case err := <-errChan:
